...
// ==================== 批量获取广告账户状态 (account_status) ==================== //
document.getElementById("getAccountStatus").addEventListener("click", async () => {
  const { accessToken: token } = await chrome.storage.session.get("accessToken");
  const textarea = document.getElementById("myTextarea");
  const resultBox = document.getElementById("resultBox");
  ...
  resultBox.innerHTML = `<p>⏳ 正在批量获取账户状态...</p>`;
  ...
  for (let i = 0; i < accountIds.length; i += 50) {
    ...
    const batch = batchIds.map(id => ({
      method: "GET",
      relative_url: `act_${id}?fields=account_status`
    }));
    ...
    try {
      ...
      res.data.forEach((item, idx) => {
        const accId = batchIds[idx];
        if (item.code === 200) {
          const body = JSON.parse(item.body);
          const statusCode = body.account_status;
      ...
      batchIds.forEach(id => {
        results.push({
          account_id: id,
          account_status: "❌ 批处理失败: " + err.message
          });
      });
    }
  }
  ...
  // 显示表格
  let html = "<h3>账户状态信息</h3><table border='1' cellpadding='5'><tr><th>账户ID</th><th>状态</th></tr>";
  results.forEach(r => {
    html += `<tr><td>${r.account_id}</td><td>${r.account_status}</td></tr>`;
  });
  html += "</table>";
  resultBox.innerHTML = html;
  ...
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "AccountStatus");
  XLSX.writeFile(wb, "AdAccounts_Status.xlsx");
});
